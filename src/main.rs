...
fn do_query(db_name: Arc<Result<String>>, lock: Arc<RwLock<()>>, args: String) -> Result<Vec<u8>> {
    ...
    while let Some(row) = rows.next()? {
        ...
        for idx in 0..col_count {
            let rv: DuckResult<DuckValue> = row.get(idx);
            match rv {
                Ok(v) => {
                    cur_row.push(util::duck_to_json(v));
                }
                Err(duckdb::Error::InvalidColumnIndex(idx)) => {
...
fn do_append(db_name: Arc<Result<String>>, lock: Arc<RwLock<()>>, args: String) -> Result<Vec<u8>> {
    let (table, rows): (String, Vec<Vec<JsonValue>>) = serde_json::from_str(&args)?;
    let name = match &*db_name {
        Ok(name) => Ok(name),
        Err(e) => Err(anyhow::Error::msg(e.to_string())),
    }?;
    ...
    {
        let app = &mut tx.appender(&table)?;
        for raw in rows.into_iter() {
            let row = util::json_row_to_params(raw);
            app.append_row(row)?;
        }
    }
    tx.commit()?;
    Ok(json!(size).to_string().into())
}
...
fn main() -> Result<()> {
    ...
    let db_name = Arc::new(env::var("DUCK_DB").map_err(|e| anyhow!("DUCK_DB {}", e)));
